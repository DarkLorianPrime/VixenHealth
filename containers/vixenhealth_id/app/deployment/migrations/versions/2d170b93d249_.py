...
def upgrade() -> None:
    ...
    op.add_column(
        "account_permissions", sa.Column("application_id", sa.UUID(), nullable=True)
    )
    op.create_foreign_key(
        None, "account_permissions", "application", ["application_id"], ["id"]
    )
    op.add_column(
        "account_roles", sa.Column("application_id", sa.UUID(), nullable=True)
    )
    op.create_foreign_key(
        None, "account_roles", "application", ["application_id"], ["id"]
    )
    op.create_unique_constraint(None, "permission", ["id"])
    op.create_unique_constraint(None, "role", ["id"])
    op.add_column(
        "role_permissions", sa.Column("application_id", sa.UUID(), nullable=True)
    )
    op.create_foreign_key(
        None, "role_permissions", "application", ["application_id"], ["id"]
    )
...
def downgrade() -> None:
    ...
    op.add_column(
        "account",
        sa.Column("access_mask", sa.VARCHAR(), autoincrement=False, nullable=False),
    )
